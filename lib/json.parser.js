...
let insertStaticHtmlInPage = (dom) => {
    ...
    let popUp = fs.readFileSync('static/html/static-html-popup.html');
    let table = fs.readFileSync('static/html/list-popup.html');

    let tagBody = dom.window.document.getElementsByTagName("body");
    tagBody[0].insertAdjacentHTML("beforebegin", popUp);
    tagBody[0].insertAdjacentHTML("beforebegin", table);

...
let insertConfigPart = (jsonObj, dom) => {
    ...
    if (jsonObj.config !== undefined) {
        ...
        let linkDefaultCss = `<link rel="stylesheet" href="${jsonObj.config.css}"></link>`;
        let linkSemantic = `<link rel="stylesheet" href="semantic.min.css"></link>`;
        let titleStr = `<title>${jsonObj.config.title}</title>`;
        tagHead[0].insertAdjacentHTML("beforeend", linkDefaultCss);
        tagHead[0].insertAdjacentHTML("beforeend", linkSemantic);
...
let insertStaticHtmlInPage = (dom) => {

    let staticPopup = fs.readFileSync('static/html/static-html-popup.html');
    let listPopup = fs.readFileSync('static/html/list-popup.html');
    let choicePopup = fs.readFileSync('static/html/choice-popup.html');
    let messagePopup = fs.readFileSync('static/html/message-popup.html');

    let tagBody = dom.window.document.getElementsByTagName("body");
    tagBody[0].insertAdjacentHTML("beforebegin", staticPopup);
    tagBody[0].insertAdjacentHTML("beforebegin", listPopup);
    tagBody[0].insertAdjacentHTML("beforebegin", choicePopup);
    tagBody[0].insertAdjacentHTML("beforebegin", messagePopup);
...
let insertMenuPart = (jsonObj, dom) => {
    ...
    let menu = `
                ...
                <input type="button" id="buttonRecap" class="buttonMenu buttonMenuRecap" value="Recupera"></input>
               `;
...
let insertScriptMenu = (jsonDoc, dom) => {
    ...
    // card.menu,js configuration
    if ( typeof cardMenu !== 'undefined') {
        // Only in card mode
        cardMenu.urlJson = "${jsonDoc.config.sendJsonCard}";
    }
    `);
...
let insertJavascriptFile = (dom) => {
    ...
    <script type="text/javascript" src="save.restore.js"></script>
    <script type="text/javascript" src="util.card.js"></script>
    <script type="text/javascript" src="util.date.js"></script>
    `;

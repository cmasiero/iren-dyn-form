...
class MandatoryRule extends GenericScript {
    ...
    buildPart() {
        ...
        let result = `
                       ...
                       //let title_display_${this.id} = document.getElementById("${this.id}").parentElement.parentElement.parentElement.previousElementSibling.style.display;
...
class HeadInit {
    buildPart() {
        return `function initialize() {
                    console.log("[initialize] Reset all border cols to default color");
...
class InitElement extends GenericScript {
    ...
    buildPart() {
        ...
        if (this.type === 'date' && this.init === 'today') {
            result = `let dt_${this.id} = new Date();

...
const resultScript = {
    ...
    scriptRule : (partHtml, type) => {
        ...
        let r = reusableRuleManager.getByName(partHtml.rule);
        if (r !== undefined) {
            partHtml.rule = r.rule;
            if (r.valMessage !== undefined) {
...
class MandatoryRule extends GenericScript {
    ...
    buildPart() {
        ...
        if (this.type === "checkbox") {
            ...
            result += `let v_${this.id} = document.getElementById("${this.id}").checked;
                      if (v_${this.id} === false){ result.indexOf("${this.valMessage}") === -1 ? result.push("${this.valMessage}") : console.log("'${this.valMessage}' already exists"); }
                     `;
            ...
            result = `let v_${this.id} = document.querySelector('input[name="${this.id}"]:checked');
                      if (v_${this.id} === null){ result.indexOf("${this.valMessage}") === -1 ? result.push("${this.valMessage}") : console.log("'${this.valMessage}' already exists"); }
                     `;
            ...
            result += `let v_${this.id} = document.getElementById('${this.id}').value;
                      if (v_${this.id} === ''){ result.indexOf("${this.valMessage}") === -1 ? result.push("${this.valMessage}") : console.log("'${this.valMessage}' already exists"); }
                      `;
    ...
    setValidationMessage() {
        this.valMessage = `Il campo ${this.label} è obbligatorio! (${this.title})`;
    }
}

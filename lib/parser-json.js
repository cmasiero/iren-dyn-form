...
const staticSync =  require('../lib/static-script-sync');
...
let insertScriptMenu = (jsonDoc, dom) => {
    ...
    let tagScript = dom.window.document.getElementsByTagName("script");
    tagScript[0].insertAdjacentHTML("beforeend", staticMenu.menu(jsonDoc.config.menuExtra));
    ...
    tagScript[0].insertAdjacentHTML("beforeend", staticMenu.menuExtra(jsonDoc.config.menuExtra));
...
let insertScriptSyncronize = (jsonDoc, dom) => {
    ...
    let tagScript = dom.window.document.getElementsByTagName("script");
    tagScript[0].insertAdjacentHTML("beforeend", staticSync.syncCardStoreNotSend(jsonDoc.config.sendJson, jsonDoc.config.sendFile));

}

let insertScriptPost = (dom) => {

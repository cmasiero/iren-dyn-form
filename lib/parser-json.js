...
let insertInitilizeData = (dom) => {
    ...
    // Init docname. Docname is declared in 'hard coded' manner inside insertHtmlHiddenField method.
    content = content.concat(`let uuid = Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15);
...
let insertScriptMenu = (dom) => {
    ...
    tagScript[0].insertAdjacentHTML("beforeend", staticMenu.send);
...
let insertScriptPost = (jsonObj, dom) => {
    ...
    let tagScript = dom.window.document.getElementsByTagName("script");
    tagScript[0].insertAdjacentHTML("beforeend", staticServer.post(jsonObj.config.send));

...
const { generatedManager } = require('./html.part');
...
class SetValueRule extends GenericScript {
    ...
    buildPart() {
        ...
        if (this.type === "hint"){
            let bc = this.buildConditionRuleValue(this.ruleValue);
            result = `
                        if (${bc.condition}){
                            // SetValueRule id ${this.id}
                            let node_${this.id} = document.getElementById('${this.id}');
                            node_${this.id}.textContent = ${this.getValue(bc)};
                        }
                   `;
            ...
            result = `
                        if (${bc.condition}){
                            ...
                            id_${this.id}.value = ${this.getValue(bc)};
                        }
                   `;
        } else if (this.type === "select"){
            // example of this.ruleValue:
            // Set a value => set if me_select0 == 'SELECT0_VAL0' : 'SELECT1_VAL0'#set if me_select0 == 'SELECT0_VAL1' : 'SELECT1_VAL1'
            // Preset values => set if me_select0 == 'SELECT0_VAL0' : 'SELECT1_VAL0'#set if me_select0 == 'SELECT0_VAL1' : '{,,AAA,aaa,BBB,aaa}'

            let arrayRuleValue = this.ruleValue.split('#');
            arrayRuleValue.forEach(rv => {
                let bc = this.buildConditionRuleValue(rv);
                let nodeValue = this.getValue(bc);
                let setOrPreset = nodeValue.includes("{") && nodeValue.includes("}") ? "PRESET" : "SET";
                let action = "";
                if (setOrPreset === "PRESET") {

                    action = `// It's a preset
                              // Makes new options
                              let nodeValue = ${nodeValue}.trim();
                              nodeValue = nodeValue.replaceAll("'",'"');
                              let newOptions = JSON.parse(nodeValue);
                              
                              // Checks if new options already exists.
                              let currentOptions = $('#${this.id} option');
                              let currentKeys = $.map(currentOptions ,function(option) { return $(option).attr("value"); });
                              let sameValues = currentKeys.every( k => newOptions[k] !== undefined );
                              let sameSize = Object.keys(newOptions).length === currentKeys.length;

                              // Changes the options if are different.
                              if (sameValues === false || sameSize === false) {

                                let $el = $("#${this.id}");
                                let currentIndex = $el.prop('selectedIndex');
                                $el.empty(); // remove old options
                                $.each(newOptions, function(key,value) {
                                    $el.append($("<option></option>").attr("value", key).text(value));
                                });
                                
                                // From card or local storage
                                if ($("#pageFrom").val() === 'card' || $("#pageFrom").val() === 'localDb'){
                                    // Workaround to initialise the "PRESET select" when a user 
                                    // recalls the card or reload from local storage
                                    $("#${this.id} option").eq(currentIndex).prop('selected', true);
                                } 
                    ...
                    action = `// It's a set
                              document.getElementById('${this.id}').value = ${nodeValue};
                              `;
                }
    ...
    getValue(bc) {
        let value = bc.type === "HARD_CODED" ? `"${bc.valueToSet}"` : `document.getElementById("${bc.valueToSet}").value`;
        // Validation, Check if variable exists.
        if (bc.type === "VARIABLE") generatedManager.getById(bc.valueToSet);
        return value;
    }

    buildConditionRuleValue(ruleValue) {

        // Example: "ruleValue": "set if ba_text00 == 'write' : 'Value to write'"
        let fc = ruleValue.substring(0,ruleValue.indexOf(':'));
        let val = ruleValue.substring(ruleValue.indexOf(':') + 1, ruleValue.length).trim();

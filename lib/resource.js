 ...
 * @param {DataSource} source
 * @returns ad array of {string, File}
 */
exports.getStaticFiles = (source) => {
    if (source === DataSource.FILESYSTEM) {

        let filenameStaticScript = fs.readdirSync(env.path_static_script);
        let resultScript = filenameStaticScript.map(filename => {
            return {
                filename: filename,
        ...
        // Static content used in card function.
        // Example: http://localhost:5000/card?uuid=gem2zri2zmc3heixpf0q1i
        let filenameStaticCard = fs.readdirSync(env.path_static_card);
        let resultCard = filenameStaticCard.map(filename => {
            return {
                filename: filename,
                file: fs.readFileSync(env.path_static_card.concat("/").concat(filename)).toString()
            }
        });

        return resultScript.concat(resultCard);

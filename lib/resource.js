 ...
 */
let config = {
    path_json_dev: "config/default_dev.json",
    path_json_test: "config/default_test.json",
    path_json_prod: "config/default_prod.json",
...
exports.getJson = (source) => {
    if (source === DataSource.FILESYSTEM) {
        ...
        let p = 'path_json_'.concat(env);
        let fileJson = fs.readFileSync(config[p]);
